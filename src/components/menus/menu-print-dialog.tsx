...
function PrintableWeekPage({
    ...
    return (
        <>
            {/* PAGE 1: Header + Menu Grid - fills entire A4 landscape page */}
            <div className="a4-print-sheet print-page-1 bg-white rounded-xl border border-slate-300 shadow-md p-6 text-slate-900 flex flex-col">
                {/* Header with Logo */}
                ...
                {/* Main Menu Grid / Table (5 Days: Seg a Sex) - flex-1 to fill remaining page */}
                            ...
                            <div key={idx} className="border border-slate-200 rounded-lg overflow-hidden bg-white flex flex-col">
                                ...
                                <div className="p-2 space-y-2.5 flex-1 flex flex-col justify-between text-xs bg-slate-50/40">
                                    {mealTitles.map((mealType, mIdx) => {
                                        ...
                                        return (
                                            <div key={mIdx} className="bg-white p-2 rounded border border-slate-100 shadow-2xs flex-1 flex flex-col">
                                                ...
                                                <p className="text-[10.5px] text-slate-700 leading-snug whitespace-pre-line break-words min-h-[28px] flex-1">
            ...
            {/* PAGE 2: Guidelines / Informational Section */}
            <div className="a4-print-sheet print-page-2 bg-white rounded-xl border border-slate-300 shadow-md p-6 text-slate-900 flex flex-col">
                {/* Small header on page 2 for context */}
                <div className="border-b-2 border-emerald-600 pb-3 mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <SchoolLogo className="h-10 w-auto" />
                        <div>
                            <h2 className="text-lg font-black text-slate-900 tracking-tight">
                                ESCOLA IBIRÁ
                                <span className="text-xs bg-emerald-100 text-emerald-800 font-bold px-2 py-0.5 rounded-full uppercase ml-2">
                                    Informações Nutricionais
                                </span>
                            </h2>
                            <p className="text-xs text-slate-600 font-medium capitalize">
                                {isUndated ? `Padrão do Mês de ${monthDisplay}` : (monthContext ? `${monthContext} (Semana ${weekNumber})` : format(weekStart, "MMMM 'de' yyyy", { locale: ptBR }))}
                            </p>
                        </div>
                    </div>
                </div>

                {/* Descriptive Guidelines - now fills page 2 */}
                <div className="bg-linear-to-br from-emerald-50/40 via-white to-amber-50/20 border border-emerald-100 rounded-xl p-5 text-slate-800 space-y-4 text-[12px] flex-1">
                    {guidelines.intro && (
                        <div className="bg-white/90 border border-emerald-100/80 rounded-lg p-4 text-slate-700 font-medium leading-relaxed whitespace-pre-line break-words text-[13px]">
                            {guidelines.intro}
                        </div>
                    )}

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Left: Pilares de Preparo */}
                        <div className="bg-white/80 border border-emerald-100/70 rounded-lg p-4 space-y-2 flex flex-col justify-start">
                            <h4 className="text-[11px] font-bold text-emerald-900 uppercase tracking-wider flex items-center gap-1.5 mb-1">
                        ...
                        <div className="space-y-3 flex flex-col justify-between">
                            <div className="space-y-2.5 text-[11px]">
                                ...
                                {guidelines.farmNote && (
                                    <div className="flex items-start gap-2 bg-emerald-50/90 border border-emerald-200/70 rounded-lg p-3 text-emerald-900">
                                        <Sprout className="h-4 w-4 text-emerald-600 shrink-0 mt-0.5" />
                                        <span className="whitespace-pre-line break-words leading-relaxed">{guidelines.farmNote}</span>
                                    </div>
                                )}
                            ...
                            {guidelines.footerNote && (
                                <p className="text-[10px] italic text-slate-500 border-t border-slate-100 pt-2 whitespace-pre-line break-words">
                                    {guidelines.footerNote}
                                </p>
                            )}
                        </div>
